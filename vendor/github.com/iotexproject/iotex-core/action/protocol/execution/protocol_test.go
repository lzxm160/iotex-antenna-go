...
type SmartContractTest struct {
	...
	Transfers    []ExecutionConfig `json:"transfers"`
}
...
func runTransfer(
	...
	fmt.Println("///////////////////transfer addr:", ecfg.RawByteCode)
	exec, err := action.NewTransfer(
		nonce+1,
		ecfg.Amount(),
		ecfg.RawByteCode,
		nil,
...
func (sct *SmartContractTest) run(r *require.Assertions) {
	...
	for _, transfer := range sct.Transfers {
		if transfer.RawByteCode == "" {
			transfer.RawByteCode = contractAddresses[transfer.ContractIndex]
		}
		receipt, err := runTransfer(bc, &transfer)
		r.NoError(err)
		r.NotNil(receipt)
		if transfer.Failed {
			r.Equal(action.FailureReceiptStatus, receipt.Status)
		...
		for _, expectedBalance := range transfer.ExpectedBalances {
			account := expectedBalance.Account
			balance, err := bc.Balance(account)
			r.NoError(err)
			r.Equal(
