...
function deposit() public payable {
    require(!paused);
    require(msg.value >= minAmount + depositFee && msg.value <= maxAmount + depositFee);

    if (safe.call.value(msg.value).gas(gasLimit)()) {
        customers.push(msg.sender);
        amounts.push(msg.value - depositFee);

        emit Receipt(msg.sender, msg.value - depositFee, depositFee);
    }
}
